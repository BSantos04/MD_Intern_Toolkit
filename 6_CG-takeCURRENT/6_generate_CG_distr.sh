...
echo 0 | gmx_20251cuda trjconv -f 3-run.xtc -o CG.xtc -s 3-run.tpr -pbc whole
cp 3-run.tpr CG.tpr
...
while [ $IBOND -lt $NBONDS ]; do
    echo $IBOND | gmx_20251cuda distance -f CG.xtc -n bonds.ndx -s CG.tpr -oall $DIR/bond_$IBOND.xvg -xvg none &> $DIR/temp_bonds.txt
    ...
    gmx_20251cuda analyze -f $DIR/bond_$IBOND.xvg -dist $DIR/distr_bond_$IBOND.xvg -xvg none -bw 0.001 
    rm -rf \#*
    let IBOND=$IBOND+1
done
...
while [ $IANG -lt $NANGLES ]; do
    echo $IANG | gmx_20251cuda angle -f CG.xtc -n angles.ndx -ov $DIR/ang_$IANG.xvg -od $DIR/temp.xvg &> $DIR/temp_angles.txt
    echo "---- ang"$IANG" ----" >> $DIR/data_angles.txt
    awk '/< angle >/ {print $5} /Std. Dev./ {print $4}' $DIR/temp_angles.txt >> $DIR/data_angles.txt
    gmx_20251cuda analyze -f $DIR/ang_$IANG.xvg -dist $DIR/distr_ang_$IANG.xvg -xvg none -bw 1.0 
    rm -rf \#*
    let IANG=$IANG+1
done
...
while [ $IDIH -lt $NDIHEDRALS ]; do
    echo $IDIH | gmx_20251cuda angle -type dihedral -f CG.xtc -n dihedrals.ndx -ov $DIR/dih_$IDIH.xvg -od $DIR/temp.xvg &> $DIR/temp_dihedrals.txt
    echo "---- dih"$IDIH" ----" >> $DIR/data_dihedrals.txt
    awk '/< angle >/ {print $5} /Std. Dev./ {print $4}' $DIR/temp_dihedrals.txt >> $DIR/data_dihedrals.txt
    gmx_20251cuda analyze -f $DIR/dih_$IDIH.xvg -dist $DIR/distr_dih_$IDIH.xvg -xvg none -bw 1.0 
    rm -rf \#*
    let IDIH=$IDIH+1
done
